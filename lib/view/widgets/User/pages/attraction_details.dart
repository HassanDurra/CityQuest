...
class AttractionDetails extends StatefulWidget {
  final String? id;
  const AttractionDetails({Key? key, this.id});
...
class _AttractionDetailsState extends State<AttractionDetails> {
  ...
  Future<void> get_attraction() async {
    var Url = Uri.parse(ApiCredientals.base_path +
        'CityQuestWEB/Attraction/single_attraction?id=${widget.id}');
    var response = await http.get(Url);
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
      if (jsonData != null) {
        setState(() {
          attraction_info = jsonData[0];
        });
      }
    }
  }
  ...
  Future<void> getReviews() async {
    var Url = Uri.parse(ApiCredientals.base_path +
        'CityQuestWEB/Attraction/getAttractionReview?type=attraction&id=${widget.id}');
    var response = await http.get(Url);
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
          ...
          SnackBar(
              content: Row(
                children: [
                  ...
                  Text('Attraction Added to Favorites!'),
                ],
              ),
          ...
          SnackBar(
              content: Row(
                children: [
                  ...
                  Text(
                      'An error occured \n while saving attraction to favorite'),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(attraction_info['title'] ?? 'Attraction Name'),
      ),
      body: Container(
        color: Colors.white,
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      attraction_info['title'] ?? "Attraction Name",
                      style: TextStyle(
                        fontSize: 30,
              ...
              Padding(
                padding: EdgeInsets.symmetric(horizontal: 16),
                child: Text(
                  attraction_info['location'] ?? "Description",
                  style: TextStyle(
                    fontSize: 15,
              ...
              Padding(
                padding: EdgeInsets.symmetric(horizontal: 16),
                child: Text(
                  attraction_info['description'] ?? "Description",
                  style: TextStyle(
                    fontSize: 15,
                    color: Colors.grey[700],
                  ),
              ...
              Padding(
                padding: EdgeInsets.symmetric(horizontal: 16),
                child: Text(
                  "Other Attractions And Activites",
                  style: TextStyle(
